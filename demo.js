...
(function () {
  ...
  var BOOTSTRAP = [
    ...
    "    # Where the pixel under the cursor lands in the workspace.",
    "    #",
    "    # This used to meet a fixed y = PLANE_Y and then clamp the result",
    "    # component by component, which is the bug you can feel: clamping moves",
    "    # the point sideways off the view ray, so the hand stops being under",
    "    # the cursor and slides along a wall of the workspace box instead.",
    "    # Two thirds of the plate did that -- 1662 of 2652 sample pixels.",
    "    #",
    "    # Clamping along the ray instead keeps it under the cursor: intersect",
    "    # the box, and take the plane crossing if it falls inside that span,",
    "    # otherwise the nearest end of it. The hand then leaves the plane",
    "    # rather than leaving the cursor, and every pixel that sees the",
    "    # workspace at all places the obstacle exactly where it was pointed.",
    "    # Pixels that see none of it return nothing, which reads as no hand.",
    "    import numpy as np",
    "    s = _ARM['scene']",
    "    o = s.CAM_POS",
    "    d = s.R_BASE_FROM_CAM @ np.array([(u - CX) / F, (v - CY) / F, 1.0])",
    "    lo, hi = 0.0, 1e9                    # slab test against the box",
    "    for i in range(3):",
    "        if abs(d[i]) < 1e-9:",
    "            if not (WS[i][0] <= o[i] <= WS[i][1]): return None",
    "            continue",
    "        a = (WS[i][0] - o[i]) / d[i]",
    "        b = (WS[i][1] - o[i]) / d[i]",
    "        if a > b: a, b = b, a",
    ...
    "    t = (PLANE_Y - o[1]) / d[1] if abs(d[1]) > 1e-9 else lo",
    "    return o + min(max(t, lo), hi) * d",
