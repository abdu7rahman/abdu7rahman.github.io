...
var GLUE = [
  "import base64, contextlib, dataclasses, io, json, sys, types",
  "import numpy as np",
  "",
  ...
  "",
  "",
  "def _base_course(index, physics):",
  "    \"\"\"The suite's own course, prepared once per (course, physics).",
  "",
  "    Straight through qlc.eval.benchmark.prepare_course: generate, truth field,",
  "    feature stack, privileged expert. Cached because the physics switch is the",
  "    only thing that changes any of it, and because it is what a re-pointed goal",
  "    is measured against.",
  "    \"\"\"",
  "    key = (index, physics)",
  "    if key not in _BASE:",
  "        cfg = course_suite(5, seed=1234)[index]",
  "        _BASE[key] = (cfg, qbench.prepare_course(cfg, _BENCH))",
  "    return _BASE[key]",
  "",
  "",
  "def _repoint(cfg, base, x, y):",
  "    \"\"\"Move the goal without regenerating the course.",
  "",
  "    qlc-goto copies the recipe with a new Pose2D and hands it back to",
  "    prepare_course, which regenerates -- and `_clear_endpoints` then paints an",
  "    0.9 m square of SMOOTH around wherever the new goal landed. That is right",
  "    for a benchmark, which must not spawn a robot mid-ice, and wrong here for",
  "    two reasons: the course you are looking at would silently change under the",
  "    click, and the terrain would no longer be the one the two torch fields were",
  "    baked against, so half the comparison would drop out on most clicks.",
  "",
  "    So the terrain is kept and only `config.goal` moves. Every array is the",
  "    same object, which is why the truth field and the feature stack are reused",
  "    rather than recomputed: both are pure functions of those arrays, the robot",
  "    and the feature spec, and none of the three has changed. The one quantity",
  "    that does depend on the goal is the privileged expert's route, so that is",
  "    recomputed, which keeps the `vs optimal` column meaning what it says.",
  "",
  "    What this gives up is the clearing. A goal you pick is on whatever the",
  "    course already had there, so it can be ice, or mud, or ground the gait",
  "    rejects -- and the caller refuses the last of those rather than pretending.",
  "    \"\"\"",
  "    moved_cfg = cfg.model_copy(update={'goal': Pose2D(x=float(x), y=float(y))})",
  "    terrain = dataclasses.replace(base.terrain, config=moved_cfg)",
  "    plan = expert_plan(terrain, _BENCH.robot, base.truth)",
  "    return qbench.Course(terrain=terrain, truth=base.truth, features=base.features,",
  "                         optimal_length=plan.length if plan.reachable else float('inf'))",
  "",
  "",
  "def qlc_course(index, physics, goal):",
  "    \"\"\"Prepare a course, optionally with the goal moved to (x, y) in metres.\"\"\"",
  ...
  "    cfg, base = _base_course(index, physics)",
  "    moved = goal is not None",
  "    _COURSE = _repoint(cfg, base, goal[0], goal[1]) if moved else base",
  "",
  "    t = _COURSE.terrain",
  "    rows, cols = t.shape",
  "    # Refused rather than carved out: the course is not re-cleared around a",
  "    # goal you set, so ground the gait rejects is genuinely unreachable. The",
  ...
  "        'goal_m': [round(t.config.goal.x, 2), round(t.config.goal.y, 2)],",
  "        'goal_material': Material(int(t.material[goal_row, goal_col])).name.lower(),",
