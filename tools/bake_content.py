...
So written.html stays the source and this writes app/src/lib/content.js from it.
(It was index.html until the lab took the root; same file, same words.)
...
def slice_sections(page: str) -> dict[str, str]:
    ...
    for sid in SECTIONS:
        ...
        if i < 0:
            sys.exit(f"bake_content: no section {sid} in written.html")
        marks.append((i, sid))
    marks.sort()
    out = {}
