...
# index.html is the lab, generated by Vite from app/index.html. Vite content
# hashes the bundle it injects, so BUNDLE below skips that, but the two
# classic scripts the page carries by hand -- the analytics beacon and the
# visit counter -- are ordinary root assets and need stamping like any other.
# It goes last so the order reads as: the written pages, then the built one.
#
# Which means the order of operations is npm run build, then this. A build
# rewrites index.html from the template and drops the stamps with it; --check
# is what catches a commit that skipped the second step.
PAGES = ("written.html", "demo.html", "404.html", "admin.html", "index.html")

# Vite's own output, named by content hash already. Stamping it would append a
# second hash that changes on every build and be dropped by the next one.
BUNDLE = re.compile(r"^/?build/")
